...
def maskrcnn_loss(mask_logits, proposals, gt_masks, gt_labels, gt_image, mask_matched_idxs):
    ...
    image_originals = []
    for img, m, p, i in zip(gt_image, gt_masks, proposals, mask_matched_idxs):
        us = nn.Upsample(size=(m.shape[1], m.shape[2]))
        image_upsampled = us(torch.repeat_interleave(torch.unsqueeze(torch.unsqueeze(img, 0), 0), 3, dim=1))
        image_upsampled = torch.repeat_interleave(image_upsampled[:,0,:,:], m.shape[0], dim=0)
        image_originals.append(project_masks_on_boxes(image_upsampled, p, i, discretization_size))
    
    labels = torch.cat(labels, dim=0)
    mask_targets = torch.cat(mask_targets, dim=0)
    image_originals = torch.cat(image_originals, dim=0)
    ...
    SMOOTH = 1e-6
    mask_threshold = 0.5
    mask_logits[torch.isnan(mask_logits)] = 0.
    mask_logits[mask_logits >= mask_threshold] = 1.
    mask_logits[mask_logits < mask_threshold] = 0.
    mask_logits_compare = mask_logits[torch.arange(labels.shape[0], device=labels.device), labels]
    vol_iou = {1:0., 2:0.}
    for label in [1, 2]:
        if len(labels[labels == label]) > 0:
            vol_intersection = torch.sum(image_originals[labels == label] * torch.minimum(mask_logits_compare[labels == label], mask_targets[labels == label]), (1, 2))
            vol_union = torch.sum(image_originals[labels == label] * torch.maximum(mask_logits_compare[labels == label], mask_targets[labels == label]), (1, 2))
            vol_iou[label] = torch.mean((vol_intersection + SMOOTH) / (vol_union + SMOOTH))
    
    mask_loss = 1. - (0.3*vol_iou[1] + 0.7*vol_iou[2])
    ...
    def forward(self,
        ...
        if self.has_mask():
            ...
            if self.training:
                ...
                gt_image = [t["image"] for t in targets]
                #print("TESTING!!!!!!")
                #print("HERE GT_MASKS[0] SHAPE")
                #print(gt_masks[0].shape)
                rcnn_loss_mask = maskrcnn_loss(
                    mask_logits, mask_proposals,
                    gt_masks, gt_labels, gt_image, pos_matched_idxs)
                '''
                rcnn_loss_mask = maskrcnn_loss(
                    mask_logits, mask_proposals,
