...
def main(args):
    ...
    if args.mode == 'testing':
        ...
        images, objects = process(device, dataset)
        print_label = 'Testing'
    else:
        ...
        images = images_original[split_list == label]
        objects = []
        for i in range(len(objects_original)):
            if split_list[i] == label:
                objects.append(objects_original[i])
    
    if device.type == 'cpu':
        images = torch.from_numpy(images).type(torch.FloatTensor)
    else:
        images = torch.from_numpy(images).type(torch.cuda.FloatTensor)
    
    dataset = GPIDataset(images, objects)
