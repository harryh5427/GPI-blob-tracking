...
def create_kitti_submission(model, iters=24, output_path='kitti_submission'):
    ...
    for test_id in range(len(test_dataset)):
        ...
        flow_pr = flow_pred[0].permute(0,1,3,2)
        flow_pr[:,0,:,:] *= 255.
        #flow_gt[0,:,:] /= 255.
...
@torch.no_grad()
def validate_synblobs(model, iters=24):
    """ Perform evaluation on the SynBlobs (test) split """
    model.eval()
    epe_list = []

    val_dataset = datasets.SynBlobs(split='validation')
    for val_id in range(len(val_dataset)):
        image1, image2, flow_gt, _ = val_dataset[val_id]
        image1 = image1[None].cuda()
        image2 = image2[None].cuda()
    ...
    epe = np.mean(np.concatenate(epe_list))
    print("Validation SynBlobs EPE: %f" % epe)
    return {'synblobs': epe}
...
def main(args):
    ...
    with torch.no_grad():
        ...
        elif args.dataset == 'synblobs':
            validate_synblobs(model.module)
