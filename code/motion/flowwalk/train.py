...
def plot_val(logger, args):
    for key in logger.val_results_dict.keys():
        ...
        plt.savefig(args.output_ckpt+f"/flowwalk_{key}.png", bbox_inches='tight')
        plt.close()
...
def plot_train(logger, args):
    ...
    plt.savefig(args.output_ckpt+"/flowwalk_train_epe.png", bbox_inches='tight')
    plt.close()
...
def main(args):
    ...
    model = nn.DataParallel(PWCLite(cfg), device_ids=args.gpus)
